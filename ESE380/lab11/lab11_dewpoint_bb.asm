...
mainpoll:
	...
	sbic PORTD, 4 ;check if TEC is OFF
	rjmp startADC  ;if so, do normal conversion

	;otherwise TEC is ON, check if heating/cooling
	sbic PORTD, 5 ;if heating up, 0, check if ambient*
	rjmp checkAmbient ;checks if atleast 70.0*
					
	sbis PORTC, 7 ;check if fan is ON
	rjmp checkDPS	;check for dewpoint output
...
checkAmbient:
	...
	cpi r4, 7		;compare msb with 7*
	brsh coolingTime	;branch if greater*		
	rjmp startADC		;continue running ADC*


coolingTime:

	sbi PORTD,5 ;control PD5 to cool	
	sbi PORTC,7 ;contol fan ON
	rjmp startADC
...
displayADC:	
	...
	mov r4, r16 ;save hex value for checking
